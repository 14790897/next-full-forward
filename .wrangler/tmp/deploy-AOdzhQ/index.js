(() => {
  ...
  async function handleEvent(event) {
    ...
    try {
      ...
      const page = await (0, import_kv_asset_handler.getAssetFromKV)(event, options);
      ...
      if (!url.pathname.startsWith("/proxy/")) {
        ...
        if (cookie) {
          const cookieObj = Object.fromEntries(
            cookie.split(";").map((cookie2) => {
              const [key, ...val] = cookie2.trim().split("=");
          ...
          if (cookieObj.current_site) {
            ...
            const actualUrl2 = new URL(actualUrlStr);
            const redirectUrl = `${url.origin}/proxy/${actualUrl2}`;
        ...
        actualUrlStr = url.pathname.replace("/proxy/", "") + url.search + url.hash;
      }
