...
async function handleRequest(request) {
  ...
  if (cookieHeaders.length > 0) {
    cookieHeaders.forEach((cookieHeader) => {
      let cookies = cookieHeader.headerValue
        .split(",")
      ...
      for (let i = 0; i < cookies.length; i++) {
        ...
        // Modify Path
        let pathIndex = parts.findIndex((part) =>
        ...
        // Modify Domain
        let domainIndex = parts.findIndex((part) =>
      ...
      for (var replace of setAttr) {
        ...
        if (!strReplace.includes(thisProxyServerUrl_hostOnly)) {
          if (!isPosEmbed(body, replace.index)) {
            var relativePath = strReplace.substring(
              match[1].toString().length,
              strReplace.length - 1
            );
            if (
              ...
              try {
                var absolutePath =
                  thisProxyServerUrlHttps +
                  new URL(relativePath, requestPathNow).href;
